...
async def player_info_embed(ctx, tag, information):
    ...
    if information == "main":
        ...
        embed = create_embed(f"Player : {player.name} ({player.tag}) (Achievements)", f"{achievements}\n[Open in Clash Of Clans]({player.share_link})", ctx.guild.me.color, "Player Information", ctx.guild.me.avatar_url)
    return embed
