...
async def player_info_embed(ctx, tag, information):
    ...
    if information == "main":
        ...
        embed = create_embed(f"Player : {player.name} ({player.tag}) (Troops)", text, ctx.guild.me.color, f"player_info|{ctx.author.id}", ctx.guild.me.avatar_url)
        ...
        embed = create_embed(f"Player : {player.name} ({player.tag}) (Achievements)", f"{achievements}\n[Open in Clash Of Clans]({player.share_link})", ctx.guild.me.color, f"player_info|{ctx.author.id}", ctx.guild.me.avatar_url)
...
async def player_info(ctx, tag, information):
    try:
        ...
        return
    embed = await player_info_embed(ctx, tag, information)
