    ...
    if raw_reaction.emoji == Emojis["Ticket"]:
        ...
        for c in guild.text_channels:
            if c.topic == str(member.id):
                embed = create_embed("You cannot do this action", "You are not allowed to create 2 ticket at the same time. You have already a ticket : " + c.mention, 0xff8000, "", guild.me.avatar_url)
        ...
        text_channel = await member.guild.create_text_channel("Ticket " + member.name, category=ticket_category, overwrites=overwrite, topic=str(raw_reaction.user_id))
